...
def evaluate(header, features, filters, hold_hour, factor_long_list, factor_short_list):
	...
	# =====计算目标结果
	# check zero
	if check_zero(factor_long_list) or check_zero(factor_short_list):
	...
	return net/int(hold_hour[:-1]), factor_long_list, factor_short_list


class alphaFactory(ea.Problem):  # 继承Problem父类
	def __init__(self, Dim, lb, ub, lbin, ubin, alldata, hold_hour):
	...
	def aimFunc(self, pop):  # 目标函数
		...
		for i in range(NIND):
			factor_list1 = []
			factor_list2 = []
			for j in range(all_factor_len):
				is_reverse = int(Vars[:, [j*4]][i])
				back_hour  = back_hour_list[int(Vars[:, [j*4+ 1]][i])]
				weight 	   = int(Vars[:, [j*4 + 2]][i]) * 0.1
				_d_index   = int(Vars[:, [j*4 + 3]][i])
				d_dum      = diff_list[_d_index - 1]

				if j < factort1_len:
					factor_list1.append((factor_classes1[j], is_reverse, back_hour, d_dum, weight))
				else:
					factor_list2.append((factor_classes2[j-factort1_len], is_reverse, back_hour, d_dum, weight))				

			header_df  = self.alldata[header_columns]
			feature_df = self.alldata[tools.convert_to_feature(factor_list1+factor_list2)].copy()
			filter_df  = self.alldata[filter_list]
			args.append([header_df, feature_df, filter_df, self.hold_hour, factor_list1, factor_list2])

		result_list = self.pool.starmap(evaluate, args)
		target_list = []
		for target, factor_long_list, factor_short_list in result_list:
			if target > self.best_score:
				self.best_score = target
				self.cache      = (factor_long_list, factor_short_list)
				# 跟踪打印最优结果
				show_r1 = []
				show_r2 = []
				for _f, _reverse_num, back_hour, d_num, weight in self.cache[0]:
					_reverse = False if _reverse_num == 0 else True
					if weight > 0:
						show_r1.append((_f, _reverse, back_hour, d_num, weight))
				for _f, _reverse_num, back_hour, d_num, weight in self.cache[1]:
					_reverse = False if _reverse_num == 0 else True
					if weight > 0:
						show_r2.append((_f, _reverse, back_hour, d_num, weight))

				print('best_score',self.best_score, show_r1, show_r2)
				line = 'best_score: ' + str(self.best_score)
				line += '\n    '
				for _f, _reverse_num, back_hour, d_num, weight in show_r1:
					_reverse = 'False' if _reverse_num == 0 else 'True'
					line += f"('{_f}', {_reverse}, {back_hour}, {d_num}, {weight})"
					line += ', '
				line += '\n    '
				for _f, _reverse_num, back_hour, d_num, weight in show_r2:
					_reverse = 'False' if _reverse_num == 0 else 'True'
					line += f"('{_f}', {_reverse}, {back_hour}, {d_num}, {weight})"
					line += ', '
...
# ====过滤重复因子
factors = set()
for _factor_name in factor_classes1 + factor_classes2:
...
factort1_len   = len(factor_classes1)
factort2_len   = len(factor_classes2)
all_factor_len = factort1_len + factort2_len
...
if __name__ == '__main__':
	...
	# ===读取数据
	df = reader.readall(trade_type, hold_hour, factors, filter_class_list=filter_class_list, date_range=(start_date, end_date))
	...
	lb   = [0, 0,   0,  0] * all_factor_len
	ub   = [1, bhl, 10, len(diff_list)] * all_factor_len
	lbin = [1, 1,   1,  1] * all_factor_len
	ubin = [1, 1,   1,  1] * all_factor_len
	Dim  = 4 * all_factor_len

	"""================================实例化问题对象==========================="""
	problem = alphaFactory(Dim, lb, ub, lbin, ubin, df, hold_hour)  # 生成问题对象
	"""==================================种群设置=============================="""
	Encoding   = 'RI'  # 编码方式
