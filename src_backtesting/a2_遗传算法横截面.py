...
def gen_selected(df, select_coin_num, direct=0):
	df1 = df.copy()
	df2 = df.copy()
	# 前置过滤
...
def evaluate(header, features, filters, hold_hour, factor_list):
	...
	return net/int(hold_hour[:-1]), factor_list
	...
	def aimFunc(self, pop):  # 目标函数
		...
		for i in range(NIND):
			...
			for j in range(len(factors)):
				...
				factor_list.append((factor, is_reverse, back_hour, d_dum, weight))

			header_df  = self.alldata[header_columns]
			feature_df = self.alldata[tools.convert_to_feature(factor_list)].copy()
...
if __name__ == '__main__':
	...
	lb   = [0, 0,   0,  0] * len(factors)
	ub   = [1, bhl, 10, len(diff_list)] * len(factors)
	lbin = [1, 1,   1,  1] * len(factors)
	ubin = [1, 1,   1,  1] * len(factors)
	Dim  = len(factors)*4
